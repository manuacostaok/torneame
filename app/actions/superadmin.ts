 ...
 */
async function setUserRole(userId: string, role: (typeof assignableRoles)[number]) {
 ...
 */
async function setUserSuspended(userId: string, suspended: boolean) {
  await assertSameOrigin();
  const session = await requireRole(["SUPERADMIN"]);
...
const wrappedSetUserRole = wrapAction(setUserRole);
const wrappedSetUserSuspended = wrapAction(setUserSuspended);
const wrappedSearchUsers = wrapAction(searchUsers);
export {
  wrappedSetUserRole as setUserRole,
  wrappedSetUserSuspended as setUserSuspended,
  wrappedSearchUsers as searchUsers,
};
